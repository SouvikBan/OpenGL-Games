...
Ring::Ring(float x, float y, float z) {
    ...
    this->rotation = 90;
    this->speed = 0;
    bool res = loadOBJ("../src/ring.obj",this->vertices, this->uvs, this->normals);
    
    GLfloat colour_buffer_data[this->vertices.size() * 3];

    for(int i=0; i<this->vertices.size();i++)
    {
        colour_buffer_data[3*i]=108.0f/256.0f;
        colour_buffer_data[3*i+1]=52.0f/256.0f;
        colour_buffer_data[3*i+2]=131.0f/256.0f;
    }
    // GLuint vertexbuffer;
