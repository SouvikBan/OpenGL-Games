...
Bomb::Bomb(float x, float y, float z) {
    ...
    this->rotation = 0;
    this->speed = 6;
    bool res = loadOBJ("../src/bomb.obj",this->vertices, this->uvs, this->normals);
    GLfloat colour_buffer_data[this->vertices.size() * 3];

    for(int i=0; i<this->vertices.size();i++)
    {
        colour_buffer_data[3*i]=153.0f/256.0f;
        colour_buffer_data[3*i+1]=163.0f/256.0f;
        colour_buffer_data[3*i+2]=164.0f/256.0f;
    }
    // GLuint vertexbuffer;
...
void Bomb::tick() {
    //this->rotation += speed;
    this->position.y -= this->speed/120.0f;
    // this->position.y += this->speed/120.0f;
    // this->position.y += this->speed/120.0f;
}
